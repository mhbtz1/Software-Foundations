...
Proof.
  unfold partial_function. unfold not. intros.
  assert (0 = 1) as Nonsense. {
    apply H with (x := 0).
    - apply eq.
    - apply eq.}
...
Theorem le_not_symmetric :
  ~ (symmetric le).
Proof.
  unfold not. unfold symmetric. intros H'. (**Idea is derive a clearly false conclusion from this false premise to show that the premise is false, and so the theorem is true *)
  assert (1 <= 0) as Random. {
...
Proof.
  intros n m p H0 H1.
  inversion H0.
  - rewrite <- H in H1. apply le_S_n in H1. assumption.
  - rewrite <- H2 in H1. apply le_S_n in H1. apply (le_trans (S n) m0 p H) in H1. apply le_Sn_le in H1. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  induction H0 as [| x' y' z' Hxy Hrest IH]. assumption. apply rt1n_trans with y'. apply Hxy. apply (IH H1).
Qed.
  - 
(** [] *)
