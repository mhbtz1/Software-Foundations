...
Check fun (n : Prop) False => n.

Definition ex_falso_quodlibet' : forall P, False -> P := fun _ False => match False with end.
(** [] *)
...
Definition eq_cons : forall (X : Type) (h1 h2 : X) (t1 t2 : list X),
  ...
  (fun X _ _ _ _ Heq Heq' =>
    match Heq with
    | eq_refl h => (match Heq' with
                     | eq_refl t => eq_refl (h :: t) end) end).
...
Definition and_assoc : forall P Q R : Prop,
  ...
  fun P Q R Hpqr => match Hpqr with
                      | conj HP (conj HQ HR) => conj (conj HP HQ) HR end.
(** [] *)

(** **** Exercise: 3 stars, standard (or_distributes_over_and) *)
Definition or_distributes_over_and : forall P Q R : Prop, P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R) :=
    
    (fun P Q R => conj (fun Hpqr => match Hpqr with
                       | or_introl HP => conj (or_introl HP) (or_introl HP)
                       | or_intror (conj HQ HR) => conj (or_intror HQ) (or_intror HR) end)
                      (fun Hpqr => match Hpqr with
                                | conj (or_introl HP) _ => or_introl HP
                                | conj _ (or_introl HP) => or_introl HP
                                | conj (or_intror HQ) (or_intror HR) => or_intror (conj HQ HR) end) ).

(** **** Exercise: 3 stars, standard (negations) *)
Definition double_neg : forall P : Prop,
    P -> ~~P (** equal to P -> ~P -> False *)
  := (fun P HP HP' =>  HP' HP). 

Definition contradiction_implies_anything : forall P Q : Prop,
    (P /\ ~P) -> Q :=
  (fun P Q HP => match HP with
                 | conj HP0 HP1 => (match HP1 HP0 with | end) end).
...
Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R) :=
(fun P Q R f HPQ => match HPQ with
                      | conj HP HQ => f HP HQ end).
(** [] *)
