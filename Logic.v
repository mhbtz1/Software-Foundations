  ...
  (** l = a' l' *)
    -   intros l'0 a. split.
        (** forward *)
         + simpl. intros [Heq | HA].
            * left. left. apply Heq.
            * apply IH in HA. apply or_assoc. right. apply HA.
         (** backward *)
         +  simpl. intros [ [Heq | Hin] | HB ].
            * left. apply Heq.
            * right. apply IH. left. apply Hin.
            * right. apply IH. right. apply HB.
Qed.
(** [] *)
...
Definition combine_odd_even (Podd Peven : nat -> Prop) : nat -> Prop := (fun n =>  if odd n then Podd n else Peven n).
...
Proof.
  ...
  destruct (odd n).
  - intros H H'. apply H. reflexivity.
  - intros H H'. apply H'. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite -> even_S.
    destruct (even n).
    + simpl. destruct IHn as [k']. rewrite -> H. exists k'. reflexivity.
    + simpl. destruct IHn as [k']. rewrite -> H. exists (S k').
      apply double_incr.
Qed.
(** [] *)
...
Proof.
  ...
  (** forward *)
   destruct b1.
     destruct b2.
        intros H. split. reflexivity. reflexivity.
        intros H. simpl in H. discriminate H.
        intros H. destruct b2. simpl in H. discriminate H.
                  simpl in H. discriminate H.
  (** backward *)
  destruct b1.
    destruct b2.
      split. intros H. simpl. apply H.
...
Proof.
  ...
  - destruct b1.
    destruct b2.
    + simpl. intros [H | H]. apply H. apply H.
    + simpl. intros [H | H]. apply H. discriminate H.
    + simpl. intros [H | H ]. discriminate H. apply H.
Qed.
(** [] *)
...
Proof.
  intros x y.
  split.
  - intros H. destruct (x =? y) eqn:H'.
    + discriminate H.
    + intros contra. rewrite <- eqb_eq in contra. rewrite -> H' in contra. discriminate contra.
  - intros H. destruct (x =? y) eqn:H'.
    + apply eqb_eq in H'. unfold not in H. apply H in H'. exfalso. contradiction.
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  split.
    ...
    induction l1.
    (** l1 = nil *)
    + destruct l2 eqn:E. reflexivity. simpl. intros H'. discriminate H'.
    + destruct l2 eqn:E. intros H'. discriminate.
    (** l1 = x :: l' *)
      simpl. intros H'.
      apply andb_true_iff with (b1 := eqb x x0) (b2 := eqb_list eqb l1 l) in H'.
      destruct H'. apply H with (a1 := x) (a2 := x0) in H0.
      rewrite -> H0. apply IHl1 in H1. rewrite -> H1. reflexivity.
    + generalize dependent l2.
      induction l1.
      - intros l2. intros H'. rewrite <- H'. simpl. reflexivity.
      - intros l2. intros H'. rewrite <- H'. simpl. apply andb_true_iff. split.
        * apply H. reflexivity.
        * apply IHl1. reflexivity.
Qed.

(** The bit I get stuck up on is when to generalize the use of a term in induction when there are several universal quantifiers. One good way to think about is that when doing induction over several quantified terms, if we don't generalize at least one, at the end we will get an inductive hypothesis which is overly specific and cannot be applied in the end to finish the proof. *)
...
Proof.
  ...
  - generalize dependent l.
    ...
    + simpl. intros H'. apply andb_true_iff in H'. destruct H'. apply IHl in H0. rewrite H. split. reflexivity. apply H0.
  - induction l.
    + simpl. intros contra. contradiction.
    + simpl. intros H'. destruct H'. apply IHl in H0. rewrite -> H. rewrite -> H0. reflexivity.
Qed.
