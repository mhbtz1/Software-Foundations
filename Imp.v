...
Fixpoint optimize_0plus_b (b : bexp) : bexp :=
  ...
  | BTrue => BTrue
  | BFalse => BFalse
  | BEq lt ri => BEq (optimize_0plus lt) (optimize_0plus ri)
  | BNeq lt ri => BNeq (optimize_0plus lt) (optimize_0plus ri)
  | BLe lt ri => BLe ( optimize_0plus lt) (optimize_0plus ri)
  | BGt lt ri => BGt ( optimize_0plus lt) (optimize_0plus ri)
  | BNot exp => BNot (optimize_0plus_b exp )
  | BAnd lt ri => BAnd (optimize_0plus_b lt) (optimize_0plus_b ri)
 end.
...
Fixpoint optimize_0plus_add (a:aexp) : aexp :=
  match a with
  | ANum n => ANum n
  | APlus e1 e2 => APlus (optimize_0plus_add e1) (optimize_0plus_add e2)
  | AMult e1 (ANum 1) => (optimize_0plus_add e1)
  | AMinus e1 e2 => AMinus (optimize_0plus_add e1) (optimize_0plus_add e2)
  | AMult e1 e2 => AMult  (optimize_0plus_add e1) (optimize_0plus_add e2)
  end.

Theorem prog_equiv: forall a1,  (aeval (optimize_0plus_add a1) = aeval (optimize_0plus a1)).
  intros a1.
  - induction a1 as [a1 IH1 | a1 IH1 a2 IH2 | a1 IH1 a2 IH2 | a1 IH1 a2 IH2 ].
    + reflexivity.
    + rewrite optimize_0plus_sound. induction a1.
Abort.   
...
| bTrueC (bv : bool): beval BTrue = bv -> bevalR BTrue bv
| bFalseC (bv : bool): beval BFalse = bv -> bevalR BFalse bv
| bEqC (a1 : aexp) (a2 : aexp) (bv : bool):  beval (BEq a1 a2) = bv -> bevalR (BEq a1 a2) bv
| bNeqC (a1 : aexp) (a2 : aexp) (bv : bool) : beval (BNeq a1 a2) = bv -> bevalR (BNeq a1 a2) bv
| bLeC (a1 : aexp) (a2 : aexp) (bv : bool): beval (BLe a1 a2) = bv -> bevalR (BLe a1 a2) bv
| bGtC (a1 : aexp) (a2 : aexp) (bv : bool) : beval (BGt a1 a2) = bv -> bevalR (BGt a1 a2) bv
| bNotC (b1 : bexp) (bv : bool) (bv: bool): beval (BNot b1) = bv -> bevalR (BNot b1) bv
| bAndC (b1 : bexp) (b2 : bexp) (bv : bool): beval (BAnd b1 b2) = bv -> bevalR (BAnd b1 b2) bv.
...
Proof.
  intros b bv.
  split.
  - intros H. induction H; try (simpl; reflexivity); try (assumption).
...
Definition state := total_map nat. (**Defines the type of a total map with keys that are nats *)
...
Proof.
  ...
  - apply E_Seq with (Y !-> 1; X !-> 0).
    + apply E_Asgn. reflexivity.
    + apply E_Asgn with (st := Y !-> 1; X !-> 0). reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  -  apply E_WhileTrue with (st' := (X !-> 1; Y !-> 2; Y !-> 0; X !-> 2) ).
     + reflexivity.
     + apply E_Seq with (st' := (Y !-> 2; Y !-> 0; X !-> 2) ); try (apply E_Asgn; reflexivity) ; try (apply E_Asgn with (st := (Y !-> 2; Y !-> 0; X !->2) ); reflexivity).
     + apply E_WhileTrue with (st' := (X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !->2) ).
       * reflexivity.
       * apply E_Seq with (st' := (Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2) );
           apply E_Asgn; reflexivity.
       * apply E_WhileFalse with (st := (X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2) );
         
           try (simpl; reflexivity);
           try (apply E_Seq with (st' := (X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !-> 2) ); reflexivity);
           try (simpl; reflexivity).
Qed.
(** [] *)
...
Proof.
  ...
  induction contra; inversion Heqloopdef; subst; simpl in H.
  - discriminate H.
...
Inductive no_whileR: com -> Prop :=
| noWhileSkip : no_whileR <{skip}>
| noWhileDef : forall x a, no_whileR <{ x := a }>
| noWhileConcat (c1 : com) (c2 : com) : (no_whileR c1) -> (no_whileR c2) -> (no_whileR <{c1;c2}>)
| noWhileUnconcat (c1 : com) (c2 : com) : no_whileR <{c1;c2}> -> no_whileR c1 -> (no_whileR c2)
| noWhileOtherUnconcat (c1 : com) (c2 : com) : no_whileR<{c1;c2}> -> no_whileR c2 -> no_whileR c1
| noWhileUnconcat1 (c1 : com) (c2 : com) : no_whileR<{c1;c2}> -> no_whileR c1
| noWhileUnconcat2 (c1 : com) (c2 : com) : no_whileR<{c1;c2}> -> no_whileR c2
| noWhileIf (c1 : com) (c2 : com) (b : bexp) : (no_whileR c1) -> (no_whileR c2) -> (no_whileR <{if b then c1 else c2 end}>)
| noWhileIfRev (c1 : com) (c2 : com) (b : bexp) : (no_whileR <{if b then c1 else c2 end}>) -> no_whileR c1 -> no_whileR c2
| noWhileIf1 (c1 : com) (c2 : com) (b : bexp) : (no_whileR <{if b then c1 else c2 end}>) -> no_whileR c1
| noWhileIf2 (c1 : com) (c2 : com) (b : bexp) : (no_whileR <{if b then c1 else c2 end}>) -> no_whileR c2.

Lemma no_whiles_concat_iff: forall c1 c2, (no_whiles <{c1 ; c2}> = true <-> no_whiles c1 = true /\ no_whiles c2 = true).
Proof.
  intros c1 c2; split; try (intros H'; simpl in H'; simpl; apply andb_true_iff in H'; assumption).
Qed.

Lemma no_whiles_cond_iff : forall c1 c2 b, (no_whiles c1 = true) /\ (no_whiles c2 = true) <->
                                     (no_whiles <{if b then c1 else c2 end}> = true).
Proof.
  intros.
  split.
  - destruct b; try (intros H';simpl;destruct H' as [H0 H1]; rewrite -> H0;rewrite -> H1;reflexivity).
  - destruct b;
     try (intros H'; simpl in H'; split;
      apply andb_true_iff in H'; destruct H' as [H' _]; assumption;
      apply andb_true_iff in H'; destruct H' as [_ H']; assumption);
      try (intros H'; simpl in H'; apply andb_true_iff in H'; assumption).
Qed.
...
Theorem no_while_terminate: forall c, no_whilesR c -> forall st, (exists st', ceval c st st').
Proof.
  intros c H.
  induction H.
  - intros st. exists st. apply E_Skip.
  - intros st. exists (x !-> (aeval st a); st). apply E_Asgn. reflexivity.
  -  intros st. destruct (IHno_whilesR1 st) as [st' H']. destruct (IHno_whilesR2 st') as [st'' H''].
    exists st''. apply E_Seq with (st' := st'); assumption.
  - intros st.  destruct (IHno_whilesR1 st) as [st' H']. destruct (IHno_whilesR2 st) as [st'' H''].   destruct (beval st b) eqn:E.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | h :: t => (match h with
               ...
               | SLoad s' => s_execute st (st s' :: stack) t
               | SPlus => (match stack with
                           | h1 :: (h2 :: t0) => s_execute st ( (h2 + h1) :: t0 ) t
                           | _ => s_execute st stack t end)
               | SMinus => (match stack with
                            | h1 :: (h2 :: t0) => s_execute st ( (h2 - h1) :: t0) t
                            | _ => s_execute st stack t end)
               | SMult =>  (match stack with
                            | h1 :: (h2 :: t0) => s_execute st ( (h2 * h1) :: t0 ) t
                            | _ => s_execute st stack t end) end)
   end.
...
Proof.
  intros. generalize dependent p2. generalize dependent stack. induction p1 as [| h' t' IH].
  - reflexivity.
  - intros stack p2. assert ( (h' :: t') ++ p2 = (h' :: (t' ++ p2) ) ) as H. {
        simpl. reflexivity.
    } rewrite -> H. induction h'.
    + simpl. apply (IH (n :: stack) (p2) ).
    + simpl. apply (IH (st x :: stack) (p2) ).
    + induction stack as [| a b IH']; try (simpl; apply (IH [] p2) ); try (induction b as [ | a' b' IH'']; try (simpl; reflexivity) ); try (simpl; apply (IH [a] p2) ); try (simpl; apply (IH ((a'+a)::b') p2) ).
    + induction stack as [| a b IH']; try (simpl; apply (IH [] p2) ); try (induction b as [ | a' b' IH'']; try (simpl; reflexivity) ); try (simpl; apply (IH [a] p2) ); try (simpl; apply (IH ((a'-a)::b') p2) ).
    + induction stack as [| a b IH']; try (simpl; apply (IH [] p2) ); try (induction b as [ | a' b' IH'']; try (simpl; reflexivity) ); try (simpl; apply (IH [a] p2) ); try (simpl; apply (IH ((a'*a)::b') p2) ).
Qed.
      (** induction p1; try (rewrite -> H; induction stack as [| h' t' IH]).
      * rewrite app_nil_l. simpl. reflexivity.
      * rewrite app_nil_l. rewrite app_nil_l in IH. simpl *)
...
Proof.
  intros.
  generalize dependent stack. induction e; try (reflexivity).
  - simpl. intros stack. rewrite (execute_app st (s_compile e1 ++ s_compile e2) [SPlus] ). rewrite (execute_app st (s_compile e1) (s_compile e2) ). rewrite (IHe1 stack). rewrite (IHe2 (aeval st e1 :: stack) ).  reflexivity. 
  -  simpl. intros stack. rewrite (execute_app st (s_compile e1 ++ s_compile e2) [SMinus] ). rewrite (execute_app st (s_compile e1) (s_compile e2) ). rewrite (IHe1 stack). rewrite (IHe2 (aeval st e1 :: stack) ).  reflexivity.
  -  simpl. intros stack. rewrite (execute_app st (s_compile e1 ++ s_compile e2) [SMult] ). rewrite (execute_app st (s_compile e1) (s_compile e2) ). rewrite (IHe1 stack). rewrite (IHe2 (aeval st e1 :: stack) ).  reflexivity.  
Qed.
...
Proof.
  intros.
  (** I omit identifiers from certain cases because these cases will be skipped by inversion *)
  inversion H as [| | | c1 c2 st0 st'0 Hbreak | c1 c2 st0 st'0 st''0 s' Hbreak | | | | | ].
  - inversion Hbreak. reflexivity.
  - inversion Hbreak.
Qed.
