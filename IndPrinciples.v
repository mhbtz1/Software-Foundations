...
Definition leaf_case (P : booltree_property_type) : Prop := forall b, P (bt_leaf b).

Definition branch_case (P : booltree_property_type) : Prop := forall b t1, P t1 -> (forall t2, P t2 -> P (bt_branch b t1 t2) ).
...
(** tree_ind: forall (X : Type) (P : tree x -> Prop), (forall x, P leaf X x) -> 
                       (forall t1 : tree X, P t1 -> (forall t2 : tree X, P t2 -> P node t1 t2) -> (forall t : tree X : P t) ) *)  
     ...
     foo'_ind :
        forall (X : Type) (P : foo' X -> Prop),
              (forall (l : list X) (f : foo' X),
                    ___P_f__________________ ->
                    ______P C1 X l f_________________   ) ->
             ___P C2_X______________________________________ ->
             forall f : foo' X, ______P f__________________
*)
...
Definition better_t_tree_ind_type : Prop :=
  (forall (X : Type) (P : t_tree X -> Prop), P (t_leaf) -> (forall (t1 : t_tree X) (X' : X), P t1 -> (forall (t2 : t_tree X), P t2 -> P (t_branch (t1, X', t2) ) ) ) -> (forall t : t_tree X, P t) ).
...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  (fun X P PL PB => fix f t := match t with
                               | t_leaf => PL
                               | t_branch (t1, X', t2) => PB t1 X' (f t1) t2 (f t2) end).
(** What is f? Its type is forall t : t_tree X, P t. How did f even get populated there?*)
Print better_t_tree_ind.
...
Proof.
  ...
  - reflexivity.
  - intros t1 X' IHn' t2 H'. simpl. rewrite IHn'. rewrite H'. reflexivity.
Qed.
