...
Fixpoint is_greater (l : nat) (m : nat) : bool :=
  match l, m with
  | S n', 0 => true
  | 0, _ => false 
  | S n', S m' => is_greater n' m'
  end.
  
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => (is_even n && is_greater n 7) ) l.
...
Fixpoint partition {X : Type}
  ...
  | h :: t => (match test h with
                 | true => (match (partition (test) (t) ) with
                              | (x, y) => (h :: x, y)
                            end)
                 | false => (match (partition (test) (t) ) with
                                         | (x, y) => (x, h::y)
                                         end) end)
  end.
...
Lemma app_map : forall (X Y : Type) (f : X -> Y) (l : list X) (x : X), app (map f l ) [f x] = map f (app l [x]).
Proof.
  intros X Y f l x.
  induction l.
  - simpl. reflexivity.
...
Proof.
  ...
  induction l.
  - simpl. reflexivity.
  - simpl. rewrite <- app_map. rewrite -> IHl. reflexivity.
(** [] *)
Qed.
...
Definition scc (n : cnat) : cnat := fun (X : Type) (succ : X -> X) (x : X) => n X succ (succ x).
...
Definition n : cnat := plus zero one.
Definition m : cnat := plus zero two.
Definition X := Type.
Definition succ (X : Type) := X -> X.
Check (n X).
