...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y l z j H0 H1.
  injection H0 as H'.
  rewrite H1 in H. rewrite <- H' in H.
  injection H as He.
  symmetry. apply He.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H0. apply eqb_0_l in H0. symmetry. apply H0.
  - intros m H0. simpl in H0. destruct m as [| m'] eqn:E.
    + discriminate H0.
...
Proof.
  ...
  - destruct m.
    + intros H0. discriminate H0.
    + intros H0. rewrite <- plus_n_Sm in H0. rewrite <- plus_n_Sm in H0.
      injection H0 as H.
...
Proof.
  ...
  generalize dependent n. (* reinserts n back into the goal as a universal quantifier *)
...
Proof.
  ...
  - intros n H0. simpl in H0. symmetry in H0. rewrite H0. reflexivity.
  - intros n H0. simpl in H0. destruct n.
    + simpl. discriminate.
    + injection H0 as H. apply IHl' in H. simpl. apply H.
(** [] *)
Qed.
(* ################################################################# *)
...
(**Notes to self:
1. Formally, how do the destruct and specialize tactics work? Coverage here is a bit vague.
**)
...
Proof.
  ...
  - intros l1 l2 H0. simpl in H0. injection H0 as H. rewrite <- H. rewrite <- H0. reflexivity.
  - destruct x as (x, y).
    destruct l1 as [| l1'].
    + intros l2 H0. simpl in H0. destruct (split l') in H0. discriminate H0.
    + destruct l2 as [| l2'].
      * intros H0. simpl in H0. destruct (split l') in H0. discriminate H0.
      * intros H0.
        simpl.
        assert (G : split l' = (l1, l2) ). {
            simpl in H0. destruct (split l').
            injection H0 as H. rewrite -> H0. rewrite -> H2.
            reflexivity.
        }
        apply IHl in G. simpl in H0. destruct (split l'). injection H0 as H.
...
(** We've now seen many of Coq's most fundamental tactics.  We'll
    ...
    Here are the ones we've seen:
      ...
      - [apply]: prove goal using a hypothesis, lemma, or constructor (backward reasoning)
  ...
  destruct (n =? m) eqn:E.
   ...
   + generalize dependent m.
     induction n as [| n' IHn'].
       - intros m.
         destruct m.
          * simpl. intros H0. discriminate H0.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: *)
      The informal structure for this proof is that let us consider the cases on if n is equal to m. If it is true, then m = n, and since (n = n) is true, and (n ?= m) is true, then (true = true) is true.
      If (n ?= m) is false, then we can reintroduce m in our goal and do induction on n and casework on m. If n is 0 and m is 0, we have a contradiction and this case cannot occur in this branch. If n is 0 and m is not zero, then we can simplify the expression and show that (0 != S n'). If n is nonzero and m is zero, then the same case is true as the last case seen before. If n is nonzero and m is nonzero, then we can just apply the inductive hypothesis given n = S n', m = S m'.
